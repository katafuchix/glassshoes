...
@implementation bestResultController
...
- (void)viewDidLoad {
	...
	NSInteger count = [gs getMaxFitCnt];
	...
	for ( int i= 0; i< count; i++) {
		...
		NSInteger right_no = [gs getMaxFitWoman:i];
		...
		rightLabel.minimumFontSize = 4;
		
		[m_scrollView addSubview:rightLabel];
		
		NSInteger left_no = [gs getMaxFitMan:i];
	...
	NSString* str = [[NSString alloc] initWithFormat:@"%d％", [gs getMaxFit]];
...
-(void)showHelpPage{
	...
	[m_helpCtrl setInfo:self left:[gs getMaxFitMan:cnt] right:[gs getMaxFitWoman:cnt] from:1];
}
