...
@implementation mutiFittingController
...
-(void)setInfo:(NSInteger)left right:(NSInteger)right{
	...
	profile* p = [gs.m_mutiLists objectAtIndex:m_nRight];
	[m_rightShoes setInfo:[p isMan] color:m_nRight data:p.m_quesLists type:4 left:false];
	...
	p = [gs.m_mutiLists objectAtIndex:left];
	[m_leftShoes setInfo:[p isMan] color:left data:p.m_quesLists type:4 left:true];
...
- (void)startTimeOut:( NSTimer *)aTimer
{
	...
	if ( m_cnt < 8 ) {
		...
		[appli.m_gpsCtrl showMutiFitResultPage:m_nLeft right:m_nRight];
...
-(IBAction)selToTop:(id)sender{
	...
	[appli.m_gpsCtrl closeMutiFittingPage];
}
